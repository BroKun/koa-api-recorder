...
app.use(bodyParser());
app.use(recorder({ dirPath: testPath, postMan: true }));
  ...
  it('测试文件生成', done => {
    request(server)
      ...
      .end((err, res) => {
        ...
        assert(fs.existsSync(path.join(testPath, 'koa-api-recorder/history.json')) === true);
        assert(fs.existsSync(path.join(testPath, 'koa-api-recorder/postman.json')) === true);
        const historyStr = fs.readFileSync(path.join(testPath, 'koa-api-recorder/history.json'), 'utf-8');
        assert(historyStr.indexOf('test body') > -1);
        assert(historyStr.indexOf('json') > -1);
        assert(historyStr.indexOf('bar') > -1);
        const postmanStr = fs.readFileSync(path.join(testPath, 'koa-api-recorder/history.json'), 'utf-8');
